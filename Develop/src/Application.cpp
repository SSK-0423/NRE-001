...
Application::Application() : _graphicsEngine(Dx12GraphicsEngine::Instance())
{
}
...
MYRESULT Application::InitTexture()
{
...
MYRESULT Application::InitConstantBuffer()
{
	ID3D12Device& device = _graphicsEngine.Device();
...
void Application::TextureMappingDraw()
{
	// 1フレームの描画処理
...
MYRESULT Application::InitOffscreenRender()
{
...
void Application::MultiPassRenderingDraw()
{
	// レンダリングコンテキスト容易
...
MYRESULT Application::InitMultiRenderTarget()
{
...
MYRESULT Application::CreateMRTPolygon()
{
	ID3D12Device& device = _graphicsEngine.Device();
...
MYRESULT Application::CreateMRTRenders()
{
	RenderTargetBufferData renderData(
		DXGI_FORMAT_R8G8B8A8_UNORM, _window->GetWindowSize().cx, _window->GetWindowSize().cy,
		{ 0.f,1.f,0.4f,1.f });
...
MYRESULT Application::CreateMRTDescriptorHeap()
{
	return _mrtDescriptorHeap.Create(_graphicsEngine.Device());
}

void Application::MultiRenderTargetDraw()
{
	// レンダリングコンテキスト用意
	RenderingContext& renderContext = _graphicsEngine.GetRenderingContext();
